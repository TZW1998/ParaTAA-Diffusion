...
def parallel_sampling(pipeline, sampler, accelerator, prompt, noise_vec_list, max_steps): 
    ...
    for i, noise in tqdm(enumerate(noise_vec_list)):
...
def main(args):
    ...
    noise_vectors_list = torch.randn(1, args.num_samples, args.timesteps + 1, 4, 64, 64, device = device, dtype = torch.float64)

    # a hack to sync noise_vectors_list from GPU 0 to all GPUs
    noise_vectors_list = accelerator.gather(noise_vectors_list)[0]
    
    if accelerator.is_main_process:
        output_dir = os.path.join(args.output_path, "SD-" + time.strftime("%Y-%m-%d-%H-%M-%S"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, default="black duck", help="The prompt use for text-to-image generation")
